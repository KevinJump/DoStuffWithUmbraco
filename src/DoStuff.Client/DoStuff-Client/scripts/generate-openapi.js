...
if (args.length < 2) {
  ...
  console.error(`Please provide the URL to the server spec as the first argument and the client name as the second argument found in ${chalk.yellow('package.json')}`);
  console.error(`Example: node generate-openapi.js ${chalk.yellow('https://localhost:44331')} ${chalk.yellow('dostuffclient')}`);
  process.exit();
}

const serverUrl = args[0];
const client = args[1];
const swaggerUrl = `${serverUrl}/umbraco/swagger/${client}/swagger.json`;
